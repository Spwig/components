...
Real-time currency exchange rates from ExchangeRatesAPI.io (APILayer)
"""
...
class ExchangeRatesAPIProvider(ExchangeRateProviderBase):
    """ExchangeRatesAPI.io provider implementation"""

    # Required class attributes
    provider_key = "exchangeratesapi"
    ...
    def capabilities(self) -> Dict[str, bool]:
        ...
        return {
            ...
            'crypto': False,
            'commodities': True,  # Supports Gold, Silver, Platinum, Palladium
            'base_currency_selection': False,  # Free tier only supports EUR base
    ...
    def credential_schema(self) -> Dict[str, Any]:
        ...
        return {
            "access_key": {
                ...
                "label": "Access Key",
                "required": True,
                "help_text": "Your ExchangeRatesAPI.io Access Key from the APILayer dashboard",
    ...
    def test_connection(self) -> Dict[str, Any]:
        ...
        """
        try:
            # Test with symbols endpoint (lightweight, doesn't count against rate limit as much)
            ...
            if response.status_code == 200:
                ...
                # ExchangeRatesAPI returns {"success": true, "symbols": {...}}
                if data.get('success'):
                    symbols = data.get('symbols', {})
                    currency_count = len(symbols) if isinstance(symbols, dict) else 0

                    return {
                        'success': True,
                        'message': f'Successfully connected to ExchangeRatesAPI.io ({currency_count} currencies available)',
                        'details': {
                    ...
                    error_info = error.get('info', 'Unknown error')
                ...
                try:
                    ...
                    message = error.get('info', 'Invalid Access Key')
                except:
            ...
            return {
                'success': False,
                'message': 'Connection error - Unable to reach ExchangeRatesAPI.io',
    ...
    def get_exchange_rate(
        ...
        If base_currency is not EUR, we fetch EUR rates and calculate conversions.
        ...
        Returns:
            ...
            Example: {'USD': Decimal('1.18'), 'GBP': Decimal('0.86')}
        ...
        if not isinstance(target_currencies, list):
            raise ValueError("target_currencies must be a list")
        ...
        try:
            # Make API request - ExchangeRatesAPI always returns rates relative to EUR
            response = requests.get(
                ...
                try:
                    ...
                    if error_data.get('success') is False:
                        ...
                        if error_code == 101:
                            ...
                            raise Exception("API function does not exist")
                        elif error_code == 104:
                            raise Exception("Rate limit reached - upgrade your plan or try again later")
                        elif error_code == 105:
                            raise Exception("Access forbidden - check your Access Key permissions")
                        elif error_code == 201:
                            raise Exception("Invalid base currency")
                        elif error_code == 202:
                            raise Exception("Invalid currency symbols")
                        else:
            ...
            if not isinstance(data, dict) or not data.get('success'):
                error = data.get('error', {})
                raise Exception(f"API error: {error.get('info', 'Invalid API response')}")
            ...
            # If base currency is EUR, directly extract target rates
            if base_currency == 'EUR':
                ...
                # Base currency is not EUR - need to convert
                # Formula: rate_GBP_USD = rate_EUR_USD / rate_EUR_GBP
                # This is the key difference from Open Exchange Rates (which uses USD)
                ...
                for currency in target_currencies:
                    ...
                    try:
                        ...
                        # Convert: if we want GBP to USD, and we have EUR to GBP and EUR to USD
            ...
            raise Exception("Connection error - unable to reach ExchangeRatesAPI.io")

        except Exception as e:
            # Re-raise with context if it's already our exception
            if "API request failed" in str(e) or "Invalid Access Key" in str(e) or "API error" in str(e):
                raise
    ...
    def get_supported_currencies(self) -> List[str]:
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                if data.get('success') and isinstance(data.get('symbols'), dict):
    ...
    def get_rate_limits(self) -> Dict[str, Any]:
        ...
        """
        # ExchangeRatesAPI doesn't provide rate limit info in response headers on free tier
        return {
            'requests_per_month': 250,  # Free tier limit
    ...
    def get_rate(self, from_currency: str, to_currency: str, date: Optional[Any] = None) -> Decimal:
        ...
        Returns:
            Exchange rate as Decimal (e.g., 1.18 means 1 EUR = 1.18 USD)
