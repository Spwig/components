...
All notable changes to the ExchangeRatesAPI.io exchange rate provider will be documented in this file.
...
### Added
- Initial release of ExchangeRatesAPI.io provider
- Support for 168+ currencies via ExchangeRatesAPI.io (APILayer)
- Real-time exchange rates with hourly updates (free tier)
- Automatic conversion for non-EUR base currencies
- Free tier support (250 requests/month)
- Encrypted credential storage (Fernet encryption)
- Comprehensive error handling for all APILayer error codes:
...
- Precious metals support (Gold XAU, Silver XAG, Platinum XPT, Palladium XPD)

### Technical Details
- Base URL: https://api.apilayer.com/exchangerates_data
- Authentication: apikey header parameter
