"""
Unit tests for ExchangeRatesAPI.io provider
"""
import unittest
...
class TestExchangeRatesAPIProvider(unittest.TestCase):
    """Test ExchangeRatesAPI.io provider implementation"""
    ...
    def setUp(self):
        ...
        self.provider = ExchangeRatesAPIProvider(self.credentials)

    def test_initialization(self):
        """Test provider initialization"""
        self.assertEqual(self.provider.provider_key, 'exchangeratesapi')
        self.assertEqual(self.provider.provider_name, 'ExchangeRatesAPI.io')
    ...
    def test_initialization_with_short_access_key(self):
        ...
        with self.assertRaises(ValueError) as cm:
            ExchangeRatesAPIProvider({'access_key': 'short'})
    ...
    def test_capabilities(self):
        ...
        self.assertTrue(capabilities['commodities'])  # Supports precious metals
    ...
    def test_get_supported_currencies_fallback(self, mock_get):
        ...
        self.assertIn('EUR', currencies)
        self.assertIn('USD', currencies)
        self.assertIn('XAU', currencies)  # Gold
    ...
    @patch('requests.get')
    def test_precious_metals_support(self, mock_get):
        """Test precious metals (Gold, Silver) are supported"""
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
            'success': True,
        ...
        rates = self.provider.get_exchange_rate('EUR', ['XAU', 'XAG', 'USD'])

        self.assertEqual(len(rates), 3)
        self.assertIn('XAU', rates)
        self.assertIn('XAG', rates)
        self.assertIn('USD', rates)

    @patch('requests.get')
    def test_error_code_103(self, mock_get):
        """Test error code 103 handling"""
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
            'success': False,
            'error': {
                'code': 103,
                'info': 'API function does not exist'
            }
        }
        mock_get.return_value = mock_response
        ...
        with self.assertRaises(Exception) as cm:
            self.provider.get_exchange_rate('EUR', ['USD'])
        self.assertIn('function does not exist', str(cm.exception))

    @patch('requests.get')
    def test_error_code_105(self, mock_get):
        """Test error code 105 handling"""
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
            'success': False,
    ...
    @patch('requests.get')
    def test_error_code_201(self, mock_get):
        """Test error code 201 handling"""
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
            'success': False,
        ...
        with self.assertRaises(Exception) as cm:
            self.provider.get_exchange_rate('EUR', ['USD'])
        self.assertIn('Invalid base currency', str(cm.exception))

    @patch('requests.get')
    def test_error_code_202(self, mock_get):
        """Test error code 202 handling"""
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
            'success': False,
        ...
        with self.assertRaises(Exception) as cm:
            self.provider.get_exchange_rate('EUR', ['USD'])
        self.assertIn('Invalid currency symbols', str(cm.exception))
