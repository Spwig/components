"""
Integration tests for ExchangeRatesAPI.io provider
These tests make actual API calls and require a valid access key
Set TEST_EXCHANGERATESAPI_ACCESS_KEY environment variable to run these tests
"""
import unittest
import os
import sys
...
from provider import ExchangeRatesAPIProvider


class TestExchangeRatesAPIIntegration(unittest.TestCase):
    """Integration tests for ExchangeRatesAPI.io"""

    @classmethod
    def setUpClass(cls):
        """Set up test fixtures for all tests"""
        cls.access_key = os.getenv('TEST_EXCHANGERATESAPI_ACCESS_KEY')

        if not cls.access_key:
            raise unittest.SkipTest(
                "TEST_EXCHANGERATESAPI_ACCESS_KEY environment variable not set. "
                "Skipping integration tests."
            )

        cls.credentials = {'access_key': cls.access_key}
        cls.provider = ExchangeRatesAPIProvider(cls.credentials)

    def test_connection(self):
        """Test actual connection to ExchangeRatesAPI.io"""
    ...
    def test_get_supported_currencies(self):
        ...
        for metal in precious_metals:
            self.assertIn(metal, currencies, f"{metal} not in supported currencies")
    ...
    def test_precious_metals_rates(self):
        """Test fetching precious metals rates (Gold, Silver)"""
        metals = ['XAU', 'XAG']  # Gold, Silver
        rates = self.provider.get_exchange_rate('EUR', metals)

        print(f"\nPrecious metals rates (EUR base):")
        for metal, rate in rates.items():
            print(f"  {metal}: {rate}")

        # Should get rates for precious metals
        for metal in metals:
            if metal in rates:  # Might not be available on all plans
                self.assertIsInstance(rates[metal], Decimal)
                self.assertGreater(rates[metal], Decimal('0'))
                # Gold and Silver rates are typically very small (troy ounce)
                self.assertLess(rates[metal], Decimal('1'))
    ...
    def test_rate_limits_info(self):
        ...
        self.assertEqual(limits['requests_per_month'], 250)
    ...
    def test_eur_to_precious_metals_conversion(self):
        """Test EUR to precious metals conversion"""
        rates = self.provider.get_exchange_rate('EUR', ['XAU', 'XAG'])

        print(f"\nEUR to precious metals:")
        for metal, rate in rates.items():
            print(f"  {metal}: {rate}")

        # Check if we got any precious metals
        if 'XAU' in rates:
            self.assertIsInstance(rates['XAU'], Decimal)
            self.assertGreater(rates['XAU'], Decimal('0'))

        if 'XAG' in rates:
            self.assertIsInstance(rates['XAG'], Decimal)
            self.assertGreater(rates['XAG'], Decimal('0'))


class TestExchangeRatesAPIErrors(unittest.TestCase):
    """Test error handling with invalid credentials"""

    def test_invalid_access_key(self):
        """Test that invalid access key raises appropriate error"""
        invalid_credentials = {'access_key': 'invalid_key_12345'}
        provider = ExchangeRatesAPIProvider(invalid_credentials)
