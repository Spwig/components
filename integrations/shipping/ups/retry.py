"""
UPS Provider Retry Logic

Implements retry decorator with exponential backoff for UPS API requests.
...
from .exceptions import UPSRateLimitError, UPSServiceUnavailableError
...
def retry_with_backoff(
    ...
    Example:
        ...
        def make_api_call():
            return requests.get('https://api.ups.com/...')
    """
    if config is None:
        config = RetryConfig()
...
def calculate_delay(
    ...
    if isinstance(exception, UPSRateLimitError) and exception.retry_after:
